...
namespace TarodevController {
    ...
    public class PlayerAnimator : MonoBehaviour {
        ...
        void Update() {
            ...
            // Flip the sprite
            if (_player.Input.Move.x != 0) transform.localScale = new Vector3(_player.Input.Move.x > 0 ? 1 : -1, 1, 1);

            // Lean while running
            var targetRotVector = new Vector3(0, 0, Mathf.Lerp(-_maxTilt, _maxTilt, Mathf.InverseLerp(-1, 1, _player.Input.Move.x)));
            ...
            _anim.SetFloat(IdleSpeedKey, Mathf.Lerp(1, _maxIdleSpeed, Mathf.Abs(_player.Input.Move.x)));
